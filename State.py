    ...
    def updateTimes(self,lowIndex):
        for i in range(self.windowStart,self.app.pbstart-self.app.splitstart-2):
            ...
            if self.currentSplits.length > i + lowIndex - self.windowStart:
                ...
                if self.diffSplits[0].get(i+lowIndex-self.windowStart).greater(Time.Time(5,timestring='-')) == -1 or self.compares[0].get(i+lowIndex-self.windowStart).equal(Time.Time(5,timestring="-")):
